...
If there is one a third for loop starts with the goal to fill in the right value (= num) for every unoccupied value (= arr[row_i][col_i]). First we check if the potential right value can be placed in the unoccupied field (is_num_possible(row_i, col_i, num, arr)). If that is so the unoccupied spot is now occupied (with num) and the recursion starts. 
